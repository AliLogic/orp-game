...
local function OnLoadServerData()
	...
	if mariadb_get_row_count() ~= 0 then
		ServerData.tax = mariadb_get_value_name_int(1, "tax")
		ServerData.tax_percent = mariadb_get_value_name_int(1, "tax_percent")
		ServerData.double_xp = mariadb_get_value_name_int(1, "double_xp")
	end
...
AddEvent('UnloadServer', function ()
	...
	local query = mariadb_prepare(sql, "UPDATE server SET tax = ?, tax_percent = ? WHERE server = 'server'", ServerData.tax, ServerData.tax_percent)
